...
export default function TestBackendScreen() {
  ...
  const testDriverAssignment = async (parcelId: string) => {
    try {
      ...
      if (assignError) {
        addResult(`❌ Driver assignment failed: ${assignError.message}`);
        return false;
      }
  ...
  const testUpdateDeliveryStatus = async (parcelId: string) => {
    try {
      ...
      if (updateError) {
        addResult(`❌ Delivery status update failed: ${updateError.message}`);
        return false;
      }
      ...
      if (deliveredError) {
        addResult(`❌ Delivery completion failed: ${deliveredError.message}`);
        return false;
      }
  ...
  const testAdminFunctions = async () => {
    try {
      ...
      if (driversError) {
        addResult(`❌ Admin: Failed to fetch drivers: ${driversError.message}`);
        return false;
      }
